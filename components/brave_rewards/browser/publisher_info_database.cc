...
void PublisherInfoDatabase::GetRecurringTipsContributions(
  ...
  info_sql.BindInt(0, month);
  info_sql.BindInt(1, year);
  info_sql.BindInt(2, ledger::REWARDS_CATEGORY::RECURRING_TIP);
...
void PublisherInfoDatabase::GetAutoContributions(
    ledger::ContributionInfoList* list,
    int32_t month,
    uint32_t year) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ...
  sql::Statement info_sql(db_.GetUniqueStatement(
      ...
      "FROM contribution_info as ci "
      "WHERE ci.month = ? AND ci.year = ? "
      "AND ci.category = ? LIMIT 1"));

  info_sql.BindInt(0, month);
  info_sql.BindInt(1, year);
  info_sql.BindInt(2, ledger::REWARDS_CATEGORY::AUTO_CONTRIBUTE);
